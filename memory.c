...
void execute_logical_commands(char *commands[], int command_count) {
    ...
    for (int i = 0; i < command_count; i++) {
        ...
        if (trimmed_command != NULL) {
            ...
            if (alias_index != -1) {
                status = execute_command(aliases[alias_index].value, &status);
            } else {
                // Replace special variables $?, $$
                char *variable;
                while ((variable = strstr(trimmed_command, "$?")) != NULL) {
                    char buffer[5];
                ...
                status = execute_command(trimmed_command, &status);
...
int main(int argc, char *argv[]) {
    ...
    // Execute commands with logical operators, aliases, and variable replacement
    char command[MAX_PATH_LEN];
    printf("Enter commands with logical operators, aliases, and variables ($?, $$):\n");
